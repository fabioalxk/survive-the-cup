...
var offerLevelPenalty = (ascension) => ascension * 1.5;
...
var generateRewardCards = (stage, rng, ascension) => {
...
// src/game/strength.ts
var bestEleven = (squad) => {
...
var expectedGoals = (attack, defense) => {
  const diff = (attack - defense) / 12;
  return Math.max(0.18, 1.5 + diff);
};
var quickResult = (homeStrength, awayStrength, rng) => {
...
var xiStrength = (state) => state.squad.reduce((s, p) => s + p.overall, 0) / state.squad.length;
...
var rngForNode = (state, nodeId) => makeRng(mixSeed(state.seed, [...nodeId].reduce((s, c) => s + c.charCodeAt(0), 0)));
...
var expirePotions = (state) => {
  ...
  refreshSquadRatings(state);
  state.activePotions = [];
  log(state, "\u{1F9EA} O efeito das po\xE7\xF5es acabou \u2014 atributos de volta ao normal.");
};
var maybeDropPotion = (state, rng) => {
  ...
  log(state, `${POTION_INFO[kind].emoji} A vit\xF3ria rendeu uma ${POTION_INFO[kind].label} \u2014 pegue-a na recompensa!`);
};
var penaltyChance = (finishing, composure, gk) => {
  const off = (finishing + composure) / 2 / 100;
  const def = gk.goalkeeping / 100;
  return clamp(0.55 + off * 0.4 - def * 0.28, 0.3, 0.95);
};
var shootoutSide = (gk, others) => ({
  gk: gk.attrs,
  takers: [...others].sort((a, b) => b.attrs.finishing - a.attrs.finishing)
});
var penaltyShootout = (home, away, rng) => {
  ...
  for (let round2 = 0; round2 < 5; round2++) {
    const ht = homeTakers[round2 % homeTakers.length];
    const at = awayTakers[round2 % awayTakers.length];
...
var matchReward = (stage, rng) => 30 + stage * 8 + rng.int(0, 15);
var attemptSalt = (state) => `:t${START_LIVES - state.lives}`;
var finishMatch = (state, homeGoals, awayGoals) => {
  const node = nodeOf(state, state.currentNodeId);
  ...
  if (draw && node.kind !== "boss") {
    const reward2 = Math.max(1, Math.round(matchReward(node.stage, rng) / 2));
    state.coins += reward2;
    state.lastMatch = { oppName, homeGoals, awayGoals, won: false, drawn: true, stage: node.stage };
    log(state, `Empate com o ${oppName} (${homeGoals}\xD7${awayGoals}). Classificado: +${reward2} moedas, sem refor\xE7o.`);
    expirePotions(state);
    clearNode(state, node);
    return;
  }
  ...
  if (!won) {
    state.lives -= 1;
    if (state.lives <= 0) {
      log(state, `Eliminado! Derrota para o ${oppName} (${homeGoals}\xD7${awayGoals}) \u2014 as vidas acabaram.`);
      state.status = "gameover";
      return;
    }
    log(state, `Derrota para o ${oppName} (${homeGoals}\xD7${awayGoals}). \u{1F494} Perdeu 1 vida \u2014 resta ${state.lives}.`);
    state.currentNodeId = null;
  ...
  state.coins += reward;
  log(state, `Vit\xF3ria sobre o ${oppName} (${homeGoals}\xD7${awayGoals})! +${reward} moedas.`);
  if (node.kind === "boss") {
    clearNode(state, node);
    state.status = "victory";
    log(state, `\u{1F3C6} ${state.managerName} venceu o chef\xE3o final!`);
    return;
  }
...
var continueAfterDefeat = (state) => {
  if (state.status !== "lifelost") return;
  state.status = "map";
};
var quickPlayNode = (state) => {
  const node = nodeOf(state, state.currentNodeId);
